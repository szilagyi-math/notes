...
\begin{notations}{Halmazok és halmazműveletek}
  ...
  A = B           & ekvivalencia
                  & \{\} = \emptyset
  \\\hline
  A \subseteq B   & részhalmaza
                  & \{1\} \subseteq \{1; 2\}
  \\\hline
  A \subset B     & valódi részhalmaza
...
\begin{notations}{Kalkulus}
  f'(x), f''(x), f^{(n)}(x)
   & első, második és $n$-edik derivált (Lagrange jelölés)
   & f'(x) = \lim_{h \to 0} \frac{f(x + h) - f(x)}{h}
  \\\hline
  \odv{f}{x}, \odv[order=2]{f}{x}, \odv[order=n]{f}{x}
   & első, második és $n$-edik derivált (Leibniz jelölés)
   & f'(x) = \odv{f}{x}
  \\\hline
  \dot f, \ddot f, \overset{n}{\dot f}
   & első, második és $n$-edik derivált (Newton jelölés)
   & \dot f = \odv{f}{t}
  \\\hline
